CREATE  Procedure [dbo].[okSP_GetLabWareMedicaidBilling_ExcludingClinicsFromTable]	@StartDate  datetime ,
															@EndDate  datetime
AS
/******************************************************************************
**		File: 
...
**      06/29/22    ETD                 Selected ClinicID from a HIPPOLYTA table
*******************************************************************************/
...
/** Populate temporary table **/
 ...
 SELECT	bs.SubmissionKey ,
   ...
   FROM LabWare_Prod.dbo.Z_BILLING_STAGING AS bs INNER JOIN
		...
		LEFT OUTER JOIN [HIPPOLYTA].[Medicaid_Lab_Billing].[dbo].[CLinicList] hcl ON hcl.clinicID = ECN.clinicid
  WHERE bs.STARTDATE BETWEEN @StartDate AND @EndDate
	AND ISNULL(bs.DEMOGRAPHICSID,0) <> 0
    AND ISNULL(bs.ENCOUNTERID,0) <> 0
	AND ECN.clinicid not in (select ClinicID from [HIPPOLYTA].[Medicaid_Lab_Billing].[dbo].[CLinicList])
    AND bs.RETRIEVED <> 'T'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[okSP_GetLabWareMedicaidBilling_ExcludingClinicsFromTable] TO [AGENCY\143452]
    AS [dbo];
