CREATE  Procedure [dbo].[okSP_GetLabWareMedicaidBilling_ExcludingClinics]	@StartDate  datetime ,
															@EndDate  datetime, @ClinicsToExclude varchar(100) 
AS
/******************************************************************************
**		File: 
...
**      06/29/22    ETD                 Get data by excluding certain clininics by using ClinicID list parameter
*******************************************************************************/
...
DECLARE @ClinicsToExcludeTable TABLE(ClinicID INT NOT NULL, PKID INT IDENTITY(1,1) NOT NULL)

	BEGIN
		INSERT INTO @ClinicsToExcludeTable(ClinicID)
		SELECT x.Data
		FROM (SELECT Data from dash.dbo.Split (@ClinicsToExclude,',') ) x
	END
...
/** Populate temporary table **/
 ...
 SELECT	bs.SubmissionKey ,
   ...
   FROM LabWare_Prod.dbo.Z_BILLING_STAGING AS bs INNER JOIN
		...
		INNER JOIN [DAT-MSQ99-340].[PHOCIS_QUERY].[dbo].[ENCOUNTERSNEW] ECN ON BS.ENCOUNTERID = ECN.ENCOUNTER_ID 
  WHERE bs.STARTDATE BETWEEN @StartDate AND @EndDate
	AND ISNULL(bs.DEMOGRAPHICSID,0) <> 0
    AND ISNULL(bs.ENCOUNTERID,0) <> 0
	AND ECN.clinicid not in (SELECT a.ClinicID FROM @ClinicsToExcludeTable a)
    AND bs.RETRIEVED <> 'T'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[okSP_GetLabWareMedicaidBilling_ExcludingClinics] TO [AGENCY\143452]
    AS [dbo];
