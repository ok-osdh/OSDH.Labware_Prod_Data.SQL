...
SELECT DISTINCT 
                         ...
                         b.Outbreak, t.ANALYSIS, t.VERSION AS Analysis_Version, CASE WHEN r.ANALYSIS = 'cult_stool' AND (r.NAME IN ('overall', 'aeromonas species', 
                         'campylobacter species', 'Escherichia coli toxigenic', 'salmonella species', 'staphylococcus aureas', 'vibrio species', 'Yersinia species')) 
                         THEN r.FORMATTED_ENTRY END AS Cult_Stool, CASE WHEN r.ANALYSIS = 'iso_aer_vib' AND (r.NAME IN ('overall', 'aeromonas/vibrio isolate Id', 
                         'other organism identified')) THEN r.FORMATTED_ENTRY END AS Iso_Aer_Vib, CASE WHEN r.ANALYSIS = 'iso_campy' AND (r.NAME IN ('overall', 'campy isolate Id', 
                         'other organism identified')) THEN r.FORMATTED_ENTRY END AS Iso_Campy, CASE WHEN r.ANALYSIS = 'iso_e_coli' AND (r.NAME IN ('overall', 'E.coli isolate Id', 
                         'other organism identified')) THEN r.FORMATTED_ENTRY END AS Iso_E_Coli, CASE WHEN r.ANALYSIS = 'iso_samomella' AND (r.NAME IN ('overall', 
                         'salmonella isolate Id', 'other organism identified')) THEN r.FORMATTED_ENTRY END AS Iso_Samomella, CASE WHEN r.ANALYSIS = 'iso_shigella' AND 
                         (r.NAME IN ('overall', 'shigella isolate Id', 'other organism identified')) THEN r.FORMATTED_ENTRY END AS Iso_Shigella, 
                         CASE WHEN r.ANALYSIS = 'iso_y_enterocol' AND (r.NAME IN ('overall', 'yersinia isolate Id', 'other organism identified')) 
                         THEN r.FORMATTED_ENTRY END AS Iso_Y_Enterocol, CASE WHEN r.ANALYSIS = 'ehec' AND 
...
WHERE        (b.Report_Link IS NOT NULL) AND (r.ANALYSIS = 'PCR_ECOLI')

GO
GRANT SELECT
    ON OBJECT::[dbo].[ENTview] TO [OSDH\LITS-EPI-SQL]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[ENTview] TO [OSDH\LITS-ENT-SQL]
    AS [dbo];
...
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
         NumPanes = 4
         Configuration = "(H (1[14] 4[5] 2[51] 3) )"
      End
   ...
   Begin DataPane = 
      ...
      Begin ColumnWidths = 52
         ...
         Width = 1500
         Width = 1500
      ', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'ENTview';
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 2, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'ENTview';
