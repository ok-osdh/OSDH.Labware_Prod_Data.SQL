CREATE TABLE [dbo].[SAMPLE] (
    ...
    [EXT_LINK]           VARCHAR (254) NULL,
    CONSTRAINT [PK_SAMPLE] PRIMARY KEY NONCLUSTERED ([SAMPLE_NUMBER] ASC),
    CONSTRAINT [FK_SAMPLE_CUSTOMER] FOREIGN KEY ([CUSTOMER]) REFERENCES [dbo].[CUSTOMER] ([NAME]),
    CONSTRAINT [FK_SAMPLE_PATIENT] FOREIGN KEY ([PATIENT], [PATIENT_REV_NO]) REFERENCES [dbo].[PATIENT] ([NAME], [REVISION_NO]),
    CONSTRAINT [FK_SAMPLE_PRACTITIONER] FOREIGN KEY ([X_PRACTITIONER]) REFERENCES [dbo].[PRACTITIONER] ([NAME])
);


GO
GRANT SELECT
    ON OBJECT::[dbo].[SAMPLE] TO [OSDH\LITS-EPI-SQL]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[SAMPLE] TO [AGENCY\MediatorApiStaging]
    AS [dbo];
